...
router.post('/auth/register', async (req: Request, res: Response) => {
  try {
    ...
    await logActivity('user.registered', { email, role: assignedRole }, String(user.id), String(user.id));
...
router.post('/auth/login', async (req: Request, res: Response) => {
  try {
    ...
    await touchLastActive(String(user.id));
    await logActivity('user.login', { email }, String(user.id), String(user.id));
...
router.get('/auth/google/callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await touchLastActive(String(user.id));
    await logActivity('user.login', { method: 'google', email }, String(user.id), String(user.id));
...
router.patch('/auth/users/:id', authenticate, async (req: Request, res: Response) => {
  ...
  await logActivity('user.updated', { changes: updates, targetEmail: target.email }, req.user!.id, String(target.id));
  return res.json({ ...updated, id: String(updated.id), hasPassword: !!updated.passwordHash, passwordHash: undefined });
});
